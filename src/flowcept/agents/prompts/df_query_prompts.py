...
def build_df_chat_rules(
    ...
    objects_summary_tool: str = "get_objects_summary",
) -> str:
    ...
    objects_summary_tool : str, optional
        Name of the tool that returns a summary of stored objects (types and columns).
    """
    return (
        ...
        "RULE 1c: The DataFrames for tasks and objects contain all records loaded for the"
        " current session. They are pre-filtered by session setup — NEVER add scope-limiting"
        " identifier filters (workflow or campaign identifiers) to your pandas code;"
        " doing so may return 0 results. For total task counts, use `len(df)` on the full df.\n"
        f"RULE 1d: If `{query_tool}` returns an error (e.g., column not found, type error),"
        f" call `fix_query(code=<failing_code>, error=<error_message>)` to obtain corrected code,"
        f" then retry with the corrected code. Never conclude data is absent after a single failed attempt.\n"
        "RULE 2: The pandas code MUST assign output to `result`."
        " Use ONLY the columns listed in the DataFrame schema below.\n"
        f"RULE 3 (OVERRIDES RULE 1 FOR ENUMERATION QUESTIONS): When the question asks for ALL"
        f" distinct values of a dimension across the workflow (the complete set, all task types,"
        f" the full execution set), your FIRST tool call MUST be `get_task_summary` — NOT"
        f" `{query_tool}`. `get_task_summary` returns the complete list of distinct task types"
        f" and per-type counts without filtering. List every returned type."
        f" When the question is compound (also includes 'best Y', 'highest Z'),"
        f" enumerate the full set from the summary FIRST, then answer the sub-question."
        f" The sub-question scopes WHICH execution to examine —"
        f" it does NOT restrict which types to include in the enumeration.\n"
        ...
        f" (stored binary objects, data files, or versioned outputs) —"
        f" your FIRST tool call MUST be `{objects_summary_tool}` to discover which object types"
        f" and metadata columns are tracked. Then call `{objects_tool}` for specific property queries."
        f" Report all returned field values verbatim. When the queried property"
        f" is absent from the objects, always report which object types ARE present"
        f" (from the summary result), then state that the specific property is not tracked.\n"
        ...
        f"RULE 7 — MANDATORY ATTRIBUTION PROTOCOL — Triggered when the question asks WHO EXECUTED,"
        f" SUBMITTED, DISPATCHED, or ASSIGNED work for an activity (the VERB is the trigger, not the named activity)."
        f" DOES trigger: 'Which agent submitted tasks for X?', 'Who dispatched X?', 'Which agent ran X?'."
        f" DOES NOT trigger: 'What data fed into X?', 'What produced artifact Y?' — for those, use objects or task query tools."
        f" When triggered:"
        f" Step 1 — Your FIRST tool call MUST be `{list_agents_tool}`. Do NOT make any other tool call first."
        f" Step 2 — Call `{query_tool}` to query the FULL record set for attribution data."
        f" Do NOT scope the query to the named activity — the dispatcher is a DIFFERENT activity;"
        f" identify it by its semantic role (the activity whose name implies submission or dispatch)."
        f" Step 3 — Map raw agent identifiers from the query result to human-readable names from Step 1."
        f" Always report BOTH the agent name and the dispatching activity name verbatim.\n"

...
def build_common_chat_rules() -> str:
    ...
    return (
        ...
        "- MANDATORY: Before answering any question or fulfilling any request about the workflow"
        " — including cases where the data may be unavailable — you MUST make at least one tool"
        " call to retrieve real data. This applies regardless of phrasing (questions, commands,"
        " listing requests, table requests). Responding without a tool call is not permitted.\n"
        ...
        " from tool results. When an exact metric is not stored but can be derived by simple arithmetic"
        " from other numeric fields returned by tools (e.g. total / count), compute and report the"
        " derived value — state which source fields were used. To find all required inputs for a"
        " derivation, query ALL applicable record types (object/artifact records, task inputs, task"
        " outputs) before concluding any component is unavailable — each record type may hold"
        " different parts of the answer. Derivation from tool results is not invention;"
        " declaring absence when a derivation is possible is an error.\n"
        ...
        "- When a question asks what data was produced or provided as input for a named activity"
        " (e.g. 'what inputs', 'what artifacts were used by', 'what data fed into'),"
        " identify the data-flow upstream producers: tasks whose outputs were consumed by that activity."
        " Do not report downstream consumers. Always include the exact activity identifier from the data.\n"
        "- When a question asks who or what SUBMITTED, DISPATCHED, or ASSIGNED tasks to run an activity"
        " (e.g. 'which agent submitted', 'which task dispatched', 'who ran this'), this is a"
        " task-attribution question — identify which agent or system dispatched the task, not"
        " which upstream task produced input data.\n"
        "- When answering a question about data properties, query ALL applicable record types"
        " (object/artifact records, task inputs, task outputs) before concluding any property"
        " is unavailable. These sources are complementary: one may hold aggregate metadata while"
        " another holds structural detail about the same data.\n"
        "- When a question asks for ALL members of a category (using 'all', 'complete', 'every',"
        " 'entire', 'full lineage') and also identifies a specific exemplar ('the one with the best X',"
        " 'around task Y'), treat the exemplar as context for WHICH workflow to examine, not as a filter"
        " on which category members to include. Report the full category set.\n"

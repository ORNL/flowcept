...
class MQDao:
    ...
    def __init__(self, mq_host=None, mq_port=None, adapter_settings=None):
        ...
        if REDIS_URI is not None:
            ...
            self._redis = Redis(
                host=MQ_HOST if mq_host is None else mq_host,
                port=MQ_PORT if mq_port is None else mq_port,
                db=0,
                password=MQ_PASSWORD if MQ_PASSWORD else None,
            )
    ...
    def send_mq_dao_time_thread_stop(
        ...
        self._redis.publish(MQ_CHANNEL, msgpack.dumps(msg))

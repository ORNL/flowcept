...
def curate_dict_task_messages(
    ...
    locally before sending to MongoDB. It also avoids tasks changing states
    once they go into finished state. This is needed because we can't
    guarantee MQ orders. Finished states have higher priority in status
    changes, as we don't expect a status change once a task goes into
    finished state. It also resolves updates (instead of replacement) of
    ...
    doc_list
        A list of docs.
    indexing_key
        The key we want to index such as task_id in tasks collection.
    """
    indexed_buffer = {}
    for doc in doc_list:

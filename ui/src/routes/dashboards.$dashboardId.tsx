/** Dashboard detail: react-grid-layout grid of charts with edit mode (drag/resize/add/remove). */
...
import { dashboardSpec, type Chart, type DashboardSpec } from "../components/dashboard/spec";
...
function DashboardDetail() {
  ...
  const onLayoutChange = useCallback(
    (layout: Layout) => {
      ...
      const next = {
        ...spec,
        layout: layout.map((l: RGLItem) => ({ chart_id: l.i, x: l.x, y: l.y, w: l.w, h: l.h })),
      };
      save.mutate(next);
    },
    [editing, spec, save],
  ...
  const gridLayout: Layout = useMemo(() => {
    if (!spec) return [];
    const placed = new Map(spec.layout.map((l) => [l.chart_id, l]));
    return spec.charts.map((c, i) => {
      const l = placed.get(c.chart_id);
      return l
        ? { i: c.chart_id, x: l.x, y: l.y, w: l.w, h: l.h }
  ...
  const removeChart = (chartId: string) => {
    save.mutate({
      ...spec,
      charts: spec.charts.filter((c) => c.chart_id !== chartId),
      layout: spec.layout.filter((l) => l.chart_id !== chartId),
    });
  };
    ...
    <div className="space-y-4 p-6">
      ...
      <div ref={containerRef as React.RefObject<HTMLDivElement>}>
        <GridLayout
          ...
          dragConfig={{ enabled: editing, cancel: ".chart-content" }}
          resizeConfig={{ enabled: editing }}
          onLayoutChange={onLayoutChange}
        >
          {spec.charts.map((c) => (
            <div key={c.chart_id} className="card flex flex-col overflow-hidden">
              <div
                className={`flex items-center justify-between border-b border-border px-3 py-1.5 ${editing ? "cursor-move" : ""}`}
              >
                <span className="text-xs font-medium">
                  {c.title || c.type}
                  {c.live && <span className="text-ok ml-2 text-[10px]">● live</span>}
                </span>
                {editing && (
                  <button onClick={() => removeChart(c.chart_id)} className="text-fg-muted hover:text-err">
                    <Trash2 size={12} />
                  </button>
                )}
              </div>
              <div className="chart-content min-h-0 flex-1">
      ...
      {spec.charts.length === 0 && (
        <div className="text-fg-muted py-16 text-center text-xs">
          Empty dashboard — switch to Edit and add a chart.
        </div>
      )}
      ...
      {adding && (
        <AddChartDialog
          ...
          onAdd={(c) => {
            save.mutate({ ...spec, charts: [...spec.charts, c] });
            setAdding(false);
          }}
        />
      )}
...
function AddChartDialog({ onAdd, onClose }: { onAdd: (c: Chart) => void; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    const chartId = `c-${Date.now().toString(36)}`;
    if (type === "markdown") {
      onAdd({ chart_id: chartId, type, title, live: false, content } as Chart);
      return;
    }
    onAdd({
      chart_id: chartId,
      ...
      <div className="card w-[440px] p-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2.5">
          <label className="flex items-center gap-2">
            ...
            <select value={type} onChange={(e) => setType(e.target.value as Chart["type"])} className={inputCls}>
